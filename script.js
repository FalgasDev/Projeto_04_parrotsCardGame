...
let numberOfCards = Number(
	prompt('Quantas cartas você quer?\ninsira um número par de 4 a 14')
);
...
function flipCard(element) {
	if (document.querySelectorAll('.turned').length < 2) {
		if (
			element
				.querySelector('.face:nth-child(2)')
				.classList.contains('turned') === false &&
			element
				.querySelector('.face:nth-child(2)')
				.classList.contains('match') === false
		) {
			...
			if (flipedCards.length === 2) {
				...
				if (firstCardSelected.innerHTML == secondCardSelected.innerHTML) {
					...
					if (matchedCards.length == pack.length - 2) {
						setTimeout(() => {
							alert(
								`Parabéns!!\nVocê ganhou em ${count} jogadas!! \nEssa partida durou: ${seconds.innerHTML} segundos`
							);
						}, 500);
						...
						setTimeout(() => {
							...
							while (rematch !== 'sim' && rematch !== 'não') {
								rematch = prompt(
									'Gostaria de reiniciar a partida? (sim ou não)'
								);
							}
					...
					setTimeout(() => {
						...
						flipedCards[0].parentNode
							.querySelector('.back')
							.classList.remove('rotate');
						flipedCards[1].classList.add('front');
						flipedCards[1].parentNode
							.querySelector('.back')
							.classList.remove('rotate');
						flipedCards[0].classList.remove('turned');
						flipedCards[1].classList.remove('turned');
